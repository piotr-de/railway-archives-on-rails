...
path = Rails.root.join("static/source")

puts "Accessing source files in #{path}"

new_classes.each do |new_class|
	filename = "#{new_class[:name].downcase}-#{new_class[:operator].downcase}-src.txt"
	...
	IO.foreach(filepath, mode: "r:iso-8859-1") do |line|
		...
		next if line.include?("<!-") || serial_no.split("-").length > 1
		new_class[:serial_no] = "#{new_class[:name]}#{separator}#{serial_no}"
...
Dir.foreach(path) do |source_file|
	next unless source_file.include?("src.txt")
	IO.foreach(File.join(path, source_file), mode: "r:iso-8859-1") do |line|
